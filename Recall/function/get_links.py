...
def find_links(driver, url, total_pages, content_keywords, skip_content, method, id):
    ...
    try:
        for i in range(1, total_pages + 1):
            ...
            if not login_bool:
                time.sleep(5)
                if check_login(driver, login_auto=True):
                    login_bool = True
            ...
            # logic pass tracking
            if method == "may_love":
                if not pass_tracking(driver=driver, link=new_url, list_css=['div.N6hSDI.row'], timewait=30):
                    continue
            else:
                if not pass_tracking(driver=driver, link=new_url, list_css=['section.shopee-search-item-result'], timewait=30):
                    continue        
            ...
            # wait final page
            if method == "may_love":
                if not pass_tracking(driver=driver, link=new_url, list_css=['div.shopee-page-controller'], timewait=30):
                    continue
            else:
                if not pass_tracking(driver=driver, link=new_url, list_css=['nav.shopee-page-controller'], timewait=30):
                    continue
